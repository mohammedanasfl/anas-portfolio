  ...
  return (
    <div
      ...
      className={`w-72 sm:w-80 rounded-[2rem] p-2 relative flex flex-col items-center hover:scale-[1.02] transition-all duration-700 z-10 ${className} ${
        isActive ? 'bg-[#ff2a2a] border-red-400 shadow-[0_20px_50px_rgba(255,42,42,0.4)]' : 'bg-white/[0.04] border border-white/10 shadow-[0_15px_40px_rgba(0,0,0,0.4)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.6)]'
      }`}
    >
      {/* The hole punch */}
      <div className="w-5 h-5 bg-gradient-to-br from-white/30 to-white/5 rounded-full shadow-[inset_0_2px_4px_rgba(0,0,0,0.5)] absolute top-4 border border-white/20 z-10 flex items-center justify-center">
      ...
      <div className={`w-full h-full rounded-[1.5rem] mt-8 p-8 flex flex-col min-h-[220px] transition-colors duration-700 ${
        ...
        <p className={`text-sm leading-relaxed font-medium transition-colors duration-700 ${
          isActive ? 'text-red-100' : 'text-white/55'
        }`}>
          {text}
        </p>
      </div>
...
const Services = () => {
  ...
  return (
    <section
      id="services"
      ref={containerRef}
      className="bg-ink pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#ffffff08_1px,transparent_1px),linear-gradient(to_bottom,#ffffff08_1px,transparent_1px)] bg-[size:80px_80px]"
    >
      <div className="max-w-6xl mx-auto relative md:h-[1350px]">
        ...
        <div data-aos="fade-up" className="md:absolute top-10 left-0 md:w-[450px] z-20 mb-16 md:mb-0">
          <div className="inline-block border border-white/15 rounded-full px-5 py-1.5 text-sm text-white/70 font-bold mb-8 bg-white/5 backdrop-blur-sm">
            How we work
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white leading-[1.1] mb-6 tracking-tight relative">
            ...
            {/* Hand-drawn arrow */}
            <svg className="absolute -bottom-10 right-10 w-12 h-12 text-brand" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <p className="text-white/55 text-base md:text-lg max-w-sm font-medium leading-relaxed">
        ...
        >
          ...
          <path
            ...
            strokeDasharray="8 10"
            mask="url(#path-mask)"
            className="drop-shadow-[0_0_6px_rgba(255,42,42,0.5)]"
          />
        </svg>
